...
import { protein_request, protein_success, protein_failure } from ".././reducers/theme.js";
...
function* protein(action) {
    try {
        ...
        yield put({
            type: protein_success.toString(),
            payload: result.data.result[0]
        });
    } catch (e) {
        yield put({
            type: protein_failure.toString(),
...
export default function* themeSaga() {
    yield takeLatest(protein_request.toString(), protein);
